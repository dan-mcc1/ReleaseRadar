          ...
          <div className="flex-1 text-center sm:text-left">
            ...
            <div className="mt-1 flex items-center gap-2 justify-center sm:justify-start flex-wrap">
              ...
              {typedDbUser?.subscription_tier === "premium" && (
                <Link
                  to="/billing"
                  className="text-[10px] font-bold uppercase tracking-wider bg-amber-400/20 text-amber-400 border border-amber-400/30 rounded-full px-2 py-0.5 hover:bg-amber-400/30 transition-colors"
                >
                  Premium
                </Link>
              )}
              {typedDbUser?.subscription_tier === "admin" && (
                <span className="text-[10px] font-bold uppercase tracking-wider bg-purple-400/20 text-purple-400 border border-purple-400/30 rounded-full px-2 py-0.5">
                  Admin
                </span>
              )}
            </div>
            ...
            {typedDbUser?.username && (
              <button
                ...
                className="mt-3 inline-flex items-center gap-1.5 text-xs font-medium text-white/70 hover:text-white bg-white/10 hover:bg-white/20 border border-white/15 rounded-full px-3 py-1.5 transition-colors"
              >
                {shareCopied ? (
                  <>
        ...
        <div className="lg:col-span-2 space-y-4">
          ...
          {shelves.length > 0 && (
            <div className="bg-neutral-800 rounded-xl overflow-hidden">
              ...
              >
                <div className="flex items-center gap-2">
                  <span className="text-base font-semibold text-white">Shelves</span>
                  <span className="text-xs text-neutral-400 bg-neutral-700 px-1.5 py-0.5 rounded-full">
                    {shelves.length}
                  </span>
                </div>
                <svg
                  className={`w-4 h-4 text-neutral-400 transition-transform duration-200 ${shelvesOpen ? "rotate-180" : ""}`}
              ...
              {shelvesOpen && (
                <div className="px-4 pb-4 border-t border-neutral-700 space-y-1 pt-3">
                  {shelves.map((shelf) => (
                    <Link
                      key={shelf.id}
                      to={`/shelves/${shelf.id}`}
                      className="flex items-center justify-between px-3 py-2 rounded-lg hover:bg-neutral-700 transition-colors group"
                    >
