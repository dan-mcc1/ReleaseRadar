...
function consequenceMessage(count: number): { text: string; color: string } | null {
  if (count === 2) {
    return {
      ...
      <div className="bg-neutral-800 border border-warning-600/60 rounded-xl p-6 w-full max-w-sm space-y-4 shadow-xl">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-warning-400 text-xs font-medium">{ordinal(me.warning_count)} warning</p>
          </div>
        </div>
        <p className="text-neutral-300 text-sm leading-relaxed">
          Your account has received a warning from our moderation team for a violation of our{" "}
          <Link to="/community-guidelines" className="underline text-neutral-200 hover:text-white">
            community guidelines
          </Link>
          .
        </p>
        {consequence && (
          <p className={`text-sm font-medium leading-relaxed ${consequence.color}`}>
            {consequence.text}
          </p>
        )}
        <button
